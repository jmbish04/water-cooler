/**
 * Migration 0001: Initial Comprehensive Schema
 *
 * Creates all tables for the AI-Curated Discovery Hub:
 * - sources: External data source configuration
 * - items: Curated content, including vectorId
 ...
 *
 * This single file replaces all previous incremental migrations.
 */
...
CREATE INDEX IF NOT EXISTS idx_actions_item ON user_actions(itemId);
CREATE INDEX IF NOT EXISTS idx_actions_user ON user_actions(userId);
CREATE INDEX IF NOT EXISTS idx_actions_user_action ON user_actions(userId, action);
...
CREATE INDEX IF NOT EXISTS idx_audit_item ON audit_logs(itemId);
...
CREATE INDEX IF NOT EXISTS idx_digest_user ON digest_history(userId);
CREATE INDEX IF NOT EXISTS idx_digest_sent ON digest_history(sentAt DESC);
